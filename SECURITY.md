...
| Global passive adversary observing all links              |    ◐     | Payloads are padded to a 256-byte lattice; transit forwards get a random 0–49 ms jitter (timing correlation); and decoy ("cover") traffic — size-matched to the real-frame lattice — obscures *when*/*how much* a node sends (`cover_traffic = off\|light\|constant`). This raises the noise floor for size/timing correlation but is **not** infeasible to defeat: there is no constant-rate shaping, so real bursts still ride above the floor. |
...
4. **Traffic analysis** — payload padding (256-byte lattice) + random 0–49 ms
   transit-forward jitter + decoy traffic (`cover_traffic`, size-matched to real
   frames) raise the bar, but a global passive adversary can still attempt
   size/timing correlation: there is no constant-rate shaping (a deliberate
   non-goal — it would break norn's lightweight, low-latency budget, and the
   per-hop jitter already costs latency). `constant` mode adds a continuous decoy
   floor for operators who accept the bandwidth cost.
